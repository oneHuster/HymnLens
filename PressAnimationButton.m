...
- (void)buttonTouchDownAndDragEnter {
    if (self.delegate && ([self.delegate respondsToSelector:@selector(touchDownByPressAnimationButton:)])) {
        
        [self.delegate touchDownByPressAnimationButton:self];
    }
    ...
    [UIView animateWithDuration:(self.toEndDuration <= 0 ? TIME_END_DURATION : self.toEndDuration) animations:^{
        ...
        if (finished == YES) {
            ...
            if (self.delegate && ([self.delegate respondsToSelector:@selector(finishedEventByPressAnimationButton:)])) {
                
                [self.delegate finishedEventByPressAnimationButton:self];
            }
        }
    }];
}
...
- (void)buttonTouchUpInside {
    if (self.delegate && [self.delegate respondsToSelector:@selector(touchUpInsideByPressAnimationButton:)]) {
        [self.delegate touchUpInsideByPressAnimationButton:self];
    }
